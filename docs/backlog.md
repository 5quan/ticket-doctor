...
| # | 问题 | 现状 | 建议 | 状态 | 优先级 |
|---|---|---|---|---|---|
| N1 | 发生时间用的是消息接收时间 | `occurredAt = message.received_at`，不是用户说的事件发生时间 | 从输入解析发生时间（ISO/自然语言），失败回退接收时间并标注 | 待办 | P0 |
| N2 | 版本用当前 HEAD | `buildCodeSource` 解析当前 SHA，不是事件发生时版本 | 按发生时间 `git rev-list -1 --before` 或环境部署记录推断；不要求用户给 SHA | 待办 | P0 |
